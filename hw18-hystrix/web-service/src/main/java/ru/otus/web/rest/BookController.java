...
public class BookController {
  ...
  public List<BookDto> getBooks() {
    return dtoRestService.getEntities("http://localhost:8003/api/book");
  }
  ...
  public BookDto edit(@PathVariable("bookId") String bookId) {
    ...
    return dtoRestService.getEntity("http://localhost:8003/api/book/id", multiValueMap, BookDto.class);
  }

  @PostMapping(value = "/api/book", consumes = MediaType.APPLICATION_JSON_VALUE)
  @ResponseBody
  public BookDto post(@RequestBody BookDto bookDto) {
    return dtoRestService.postEntity("http://localhost:8003/api/book", buildBookFromDto(bookDto), BookDto.class);
  }

  @PutMapping(value = "/api/book", consumes = MediaType.APPLICATION_JSON_VALUE)
  public BookDto put(@RequestBody BookDto bookDto) {
    return dtoRestService.putEntity("http://localhost:8003/api/book", buildBookFromDto(bookDto), BookDto.class);
  }
  ...
  public ResponseEntity<Void> delete(@PathVariable("bookId") String bookId) {
    MultiValueMap<String, String> multiValueMap = new LinkedMultiValueMap<>();
    multiValueMap.add("bookId", bookId);
    dtoRestService.deleteEntity("http://localhost:8003/api/book/id", multiValueMap, BookDto.class);

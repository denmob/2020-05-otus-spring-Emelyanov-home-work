...
public class UserChangelog {
  ...
  public void addUser(MongockTemplate template) {
    var user = User.builder().username("user").password(bCryptPasswordEncoder.encode("12345")).role("ROLE_USER").build();
    template.save(user);
  }

  @ChangeSet(order = "002", id = "addAdmin", author = "dyemelianov", runAlways = true)
  public void addAdmin(MongockTemplate template) {
    var admin = User.builder().username("admin").password(bCryptPasswordEncoder.encode("67890")).role("ROLE_ADMIN").build();
    template.save(admin);
  }

  @ChangeSet(order = "003", id = "addTest", author = "dyemelianov", runAlways = true)
  public void addTestAcl(MongockTemplate template) {
    var test = User.builder().username("test").password(bCryptPasswordEncoder.encode("123")).role("ROLE_ACL").build();
    template.save(test);
  }
}
